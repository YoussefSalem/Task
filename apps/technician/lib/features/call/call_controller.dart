...
/// Everything the call screen needs. [roomId] is the LiveKit room the server
/// authorizes — it is the technician's offer id, which both the pro and the
/// customer join to talk about a job.
@immutable
class CallArgs {
  const CallArgs({required this.roomId, required this.peerName});

  final String roomId;
  final String peerName;

  @override
  bool operator ==(Object other) => other is CallArgs && other.roomId == roomId;
...
class CallController
  ...
  Future<void> _connect(CallArgs arg) async {
    try {
      ...
      final result = await fn.call<Map<Object?, Object?>>(<String, Object?>{
        'offerId': arg.roomId,
      });
      ...
      state = AsyncData(
          CallState(phase: CallPhase.error, errorMessage: e.toString()));
    }
  }
