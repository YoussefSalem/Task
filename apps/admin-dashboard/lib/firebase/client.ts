...
export function getFirebaseClient(){if(services)return services;const status=firebaseConfigStatus();if(!status.configured)throw new Error(`Firebase configuration is missing: ${status.missing.join(", ")}`);const app=getApps().length?getApp():initializeApp(config);services={app,auth:getAuth(app),db:getFirestore(app),storage:getStorage(app)};return services}
